...
function update($link, $linksystem, $controller, $method, $varPost, $varGet, $varSession, $credentials, $dayoftheToday, $nowTime, &$systemErrorMessage, &$datacontrolsystem){
  ...
  $userReceivedData=primarydiagnosisDataPattern($link, $credentials['Mode'], $varPost); 
...
if(!validateUserReceivedData($userReceivedData, $link, $systemErrorMessage, $varPost, $controller, $varSql)){
  ...
  if(!empty($getcomparedate=comparedata($userReceivedData, getDataPrimarydiagnosisDatabase($link, filter_var($varPost['id']), $credentials['Mode']), $link, $varPost, $controller, $varSql))){

    $datacontrolsystem= array('id_source' => $varPost['id'], 'sourcetable' => 'primarydiagnosis', 'currentTime' => $dayoftheToday." ".$nowTime, 'actionperformed' => 'update', 'id_user' => $credentials['IdServidor'], 'changeddata' => $getcomparedate, 'mode' => $credentials['Mode']);
...
if(!validateUserReceivedData($userReceivedData, $link, $systemErrorMessage, $varPost, $controller, $varSql)){
   ...
   return(manufactureComponentContainer(6,manufactureComponentAlert('danger', $systemErrorMessage).manufactureComponentPageBodyTitle('CADASTRO DE DIAGNOSTICOS',null, null).manufactureComponentFormPrint($linksystem, 'primarydiagnosis', 'edit',(array_map('manufactureComponentForm', $userReceivedData))).manufactureComponentButtonReturn($link, $linksystem, $credentials['Login'], 'primarydiagnosis', 'edit', 'save')));
    ...
    // var_dump($varSql);
     ...
     $datacontrolsystem= array('id_source' => getIDprimarydiagnosis($link, $varSql), 'sourcetable' => 'primarydiagnosis', 'currentTime' => $dayoftheToday." ".$nowTime, 'actionperformed' => 'insert', 'id_user' => $credentials['IdServidor'], 'changeddata' => null, 'mode' => $credentials['Mode']);
